...
コマンドラインから部屋に参加し、**カレントディレクトリのファイルを共有**しつつ、
他の参加者のファイルを **`./shared` にダウンロード＆再シード**する。写真・動画に
限らず zip や書類など任意のファイルを扱える。ブラウザ版と同じ部屋・同じ index に
そのまま相乗りできる。
...
- カレント直下のファイル（隠しファイルを除く）を content hash（SHA-256）で
  識別し、`add` で index に載せる。ブラウザ版と id が一致するので重複は自動
  dedup される。
...
- サムネイルは軽量なプレースホルダ SVG（種別アイコン＋拡張子ラベル。画像=🖼️ /
  動画=🎬 / 音声=🎵 / 書庫=🗜️ / その他=📄）。実サムネ生成には画像ライブラリ／
  ffmpeg が要り単一バイナリ配布と両立しないため省略。
